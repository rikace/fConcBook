...
namespace Functional.CSharp.Concurrency.Tasks
{
    public static class TaskEx
    {
            ...
            Task<TMid> item2) // #A
        {
            ...
            var lifted1 = Pure(selector.Curry()); // #C
            var lifted2 = Apply(item1, lifted1); // #D
            return Apply(item2, lifted2); // #D
        }

        public static Task<TOut> LifTOut<TIn, TMid1, TMid2, TOut>(Func<TIn, TMid1, TMid2, TOut> selector,
            Task<TIn> item1, Task<TMid1> item2, Task<TMid2> item3) // #A
        {
        ...
        {
            return input.ContinueWith(t => map(t.Result));
        }
        ...
        public static Task<T> Return<T>(this T input)
        {
            return Task.FromResult(input);
        }

        public static Task<T> Pure<T>(T input)
        {
            return Task.FromResult(input);
        }
        ...
        public static Task<Func<TMid, TOut>> Apply<TIn, TMid, TOut>(this Task<Func<TIn, TMid, TOut>> liftedFn,
            Task<TIn> input)
        {
            return input.Apply(liftedFn.Fmap(Functional.Curry));
        }
