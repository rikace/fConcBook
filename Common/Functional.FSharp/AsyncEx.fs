...
module AsyncHelpers =
    ...
    type Microsoft.FSharp.Control.Async<'a> with
        ...
        static member StartContinuation (computation:Async<'a>, cont: 'a -> unit) =
...
module rec AsyncOperators =
    ...
    let (<*>) (funAsync:Async<'a -> 'b>) (opAsync:Async<'a>) = AsyncEx.apply funAsync opAsync
    // <!> : f:('a -> 'b) -> m:Async<'a> -> Async<'b>
    let (<!>) (map : 'a -> 'b) (value : Async<'a>) = AsyncEx.map map value
    
    let (<^>) (value:'a) = AsyncEx.``pure`` value
