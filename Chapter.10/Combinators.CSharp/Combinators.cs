    ...
    public static class Combinators
    {
        ...
        public static async Task CombinatorRedundancy()
        {
            ...
            Func<string, string, string, CancellationToken, Task<string>> GetBestFlightAsync =
                ...
                {
                    var url = $"flight provider {carrier}";
            ...
            var recommendationFlight = await Task.WhenAny(recommendationFlights); // #D
        ...
        private static Task ForEachAsync<T>(this IEnumerable<T> source, int maxDegreeOfParallelism, Func<T, Task> body)
        {
            return Task.WhenAll(
        ...
        private static async Task SendEmailsAsync(List<string> emails)
        {
            var client = new SmtpClient();

            Func<string, Task> sendEmailAsync = async emailTo =>
            {
                var message = new MailMessage("me@me.com", emailTo);
