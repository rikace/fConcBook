            ...
            endpoint.ToConnectClientObservable()
                .Subscribe(client => {
                        GetClientStream(client, sslName)
                            ...
                            .Select(rawData => Deserialize<StockData>(formatter, rawData.Array))
                            // using grouppBy, we are filtering this data into multiple observable. Each observable can have its own unique operations. Because of grouping, Throttling now acts independently on each stock symbol. only Stocks with identical StocksSymbols will be filtered within the given throttle time span.
                            //Thottling can be done based on the stream data itself (rather than just a timespan). For Example, in the follwong code, the DistinctUntilChanged operator filters on a specific field.
                            ...
                            .SelectMany(group =>
                                group
                                    ...
                                    //     .DistinctUntilChanged(stock => Math.Ceiling(stock.High/100))
