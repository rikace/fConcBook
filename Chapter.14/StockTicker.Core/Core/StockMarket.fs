...
module StockMarket =
    ...
    type StockMarket (initStocks : Stock array) =   // #A
        ...
        let startStockTicker (stockAgent : Agent<StockTickerMessage>) =             
            Observable.Interval(TimeSpan.FromMilliseconds 200.0)
        ...
        static let instanceStockMarket =
            System.Lazy<StockMarket>.Create(fun () -> StockMarket(Stock.InitialStocks()))
                ...
                let rec marketIsOpen (stocks : Stock array)   // #F
                    ...
                    | UpdateStockPrices ->  // #E
                        ...
                        |> PSeq.iter(fun stock ->        // #H
                            ...
                            isStockChanged
                            |> Option.iter(fun updatedStock ->
                                subject.OnNext(Trading.UpdateStock(updatedStock))))
                        return! marketIsOpen stocks stockTicker
                    | CloseMarket(c) ->
                        stockTicker.Dispose()
        ...
        member this.GetMarketState(connId) =
            stockMarketAgent.PostAndAsyncReply(fun ch -> GetMarketState(connId, ch))
