...
namespace StockTicker.Server.CSharp
{
    ...
    public static class ResultExtensions
    {
        ...
        public static Result<T> ToResult<T>(this FSharpResult<T, string> fsResult)
            where T : class
        {
            if (fsResult.IsOk) return Ok(fsResult.ResultValue);
            return new Exception(fsResult.ErrorValue);
        }

        public static Result<T> ToResult<T>(this FSharpOption<T> option)
        {
            if (option != null) return Ok(option.Value);
    ...
    public static class ResultExt
    {
        ...
        {
            return result.IsOk ? okMap(result.Ok) : failureMap(result.Error);
        }
