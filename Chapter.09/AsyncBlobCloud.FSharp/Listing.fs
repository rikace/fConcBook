namespace AsyncBlobCloud.FSharp
...
module Listing = 
    ...
    module Helpers =
        ...
        let getListBlobMediaSync (container:CloudBlobContainer) =
            let blobToken = new BlobContinuationToken()
            
            let blobs = container.ListBlobsSegmentedAsync(blobToken).Result
            blobs.Results
            |> Seq.map(fun blob ->
                blob.Uri.Segments.[blob.Uri.Segments.Length - 1])

        let getListBlobMediaAsync (container:CloudBlobContainer) = async {
            let blobToken = new BlobContinuationToken()
    ...
    module CodeSnippets =
        ...
        let downloadMediaCompAsync (container:CloudBlobContainer)
            ...
            let! (blobStream : Stream) = blockBlob.OpenReadAsync()
            return Image.Load<Rgba32>(blobStream) // #C
        }
        ...
        let downloadMediaCompAsyncParallel() = retry {     // #B
            let! container = getCloudBlobContainerAsync()  // #F
            let blobToken = BlobContinuationToken()
            let! computations =
                container.ListBlobsSegmentedAsync(blobToken) // #G
                |> TaskEx.map(fun segment ->
                            segment.Results
                            |> Seq.map(transformAndSaveImage container)) // #H
        ...
        let parallelComp() = async {
            let blobToken = BlobContinuationToken()
            let! blobs = container.ListBlobsSegmentedAsync(blobToken) |> Async.AwaitTask           
            return
                blobs.Results
                |> Seq.map(downloadMediaCompAsync container)
                |> Async.Parallel
        }
        ...
        let downloadMediaCompAsyncParallelThrottle() = async {
            ...
            let blobToken = BlobContinuationToken()
            let! computations = container.ListBlobsSegmentedAsync(blobToken) // #C
                                |> Async.AwaitTask     
            return!  
                computations.Results   
                |> Seq.map(fun blobMedia -> transformAndSaveImage container blobMedia)
                |> Async.parallelWithThrottle maxConcurrentOperations // #D
        }
