...
namespace QuickSort.CSharp
{
    public static class Functions
    {
        ...
        {
            var pivotPos = (right + left) / 2; // often a random index between left and right is used
            var pivotValue = items[pivotPos];
        ...
        private static void QuickSort_Parallel<T>(T[] items, int left, int right) where T : IComparable<T>
        {
            ...
            var pivot = Partition(items, left, right);
            var leftTask = Task.Run(() => QuickSort_Parallel(items, left, pivot));
            var rightTask = Task.Run(() => QuickSort_Parallel(items, pivot + 1, right));
            Task.WaitAll(leftTask, rightTask);
        }

        public static void QuickSort_Parallel_Threshold<T>(T[] items) where T : IComparable<T>
        {
            var maxDepth = (int) Math.Log(Environment.ProcessorCount, 2.0);
